...
func HeapSortFloat(a []float64, y, z int) []float64 {
	first := y
	low := 0
	high := z - y
	...
	// build heap
	for i := (high - 1)/2; i >= 0; i-- {
		a = heapifyFloat(a, i, high, first)
	}

	// pop into end of data
	for i := high - 1; i >= 0; i-- {
		a[first], a[first+i] = a[first+i], a[first]
		a = heapifyFloat(a, low, i, first)
	}
...
func heapifyFloat(a []float64, low, high, first int) []float64 {
	root := low
	for {
		child := 2 * root + 1
		if child >= high {
